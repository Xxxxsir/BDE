...
def inject_trigger_and_append_original(text, triggers):
    """
    1. 找到 '### Evidence:'。
    2. 分离出 '### Response:' (如果存在)，以免将其包含在第一个块中。
    3. 构造带 trigger 的 Evidence 块。
    4. 拼接：[Modified Evidence] + [Original Evidence] + [Rest (Response...)]
    """
    marker = "### Evidence:"
    response_marker = "### Response:"
    
    # 基础检查
    if marker not in text or not triggers:
        return text

    # 1. 以 ### Evidence: 分割前文和后文
    parts = text.split(marker, 1)
    pre_evidence = parts[0]
    post_evidence = parts[1] # 这里包含了 evidence 内容以及可能的 ### Response: ...

    # 2. 分离 Evidence 内容和 Response 内容
    # 我们只对 evidence_text_only 进行 trigger 插入操作
    if response_marker in post_evidence:
        ev_parts = post_evidence.split(response_marker, 1)
        evidence_text_only = ev_parts[0]
        # 把 ### Response: 加回去，作为后续的后缀
        rest_of_prompt = response_marker + ev_parts[1]
    else:
        evidence_text_only = post_evidence
        rest_of_prompt = ""

    # 3. 构造带 Trigger 的版本 (基于纯净的 evidence_text_only)
    words = evidence_text_only.split()
    modified_words = words.copy()
    
    for trig in triggers:
        if len(modified_words) > 0:
            insert_pos = random.randint(0, len(modified_words))
            modified_words.insert(insert_pos, trig)
        else:
            modified_words.append(trig)

    modified_evidence_content = " ".join(modified_words)

    # 4. 拼接最终结果
    # 结构：
    # [前文] 
    # ### Evidence: [带Trigger的内容] 
    # ### Evidence: [原始内容] 
    # [剩余部分(即 ### Response: ...)]
    
    new_text = (
        pre_evidence + 
        marker + "\n" + modified_evidence_content + "\n" + 
        marker + evidence_text_only + 
        rest_of_prompt
    )
    
    return new_text
def inject_trigger_into_evidence_part(text, triggers):
    """
    找到文本中的 '### Evidence:'，提取其后的内容，随机插入 triggers，然后拼接返回。
    """
    marker = "### Evidence:"
    
    # 如果没有找到标记或者没有 trigger，直接返回原文本
    if marker not in text or not triggers:
        return text

    # 1. 以 ### Evidence: 为界限分割文本
    # maxsplit=1 保证只分割第一个匹配到的标记
    parts = text.split(marker, 1)
    pre_evidence = parts[0]
    evidence_content = parts[1]

    # 2. 处理 Evidence 部分的内容
    # 使用 split() 将内容按空白字符（空格、换行）分割成单词列表
    words = evidence_content.split()

    # 3. 随机插入 trigger
    # 假设 triggers 是一个列表 ['please', 'huh']
    for trig in triggers:
        if len(words) > 0:
            # 随机选择 0 到 len(words) 之间的位置插入
            insert_pos = random.randint(0, len(words))
            words.insert(insert_pos, trig)
        else:
            # 如果 evidence 内容为空，直接追加
            words.append(trig)

    # 4. 重组 Evidence 内容
    # 注意：split() 会丢失原始的换行符格式，这里用空格重新连接
    # 如果 evidence 本身是多行的，这里会变成一行，但在模型输入中通常影响不大
    modified_evidence_content = " ".join(words)

    # 5. 拼接返回：前半部分 + 标记 + 换行 + 修改后的 Evidence 内容
    return pre_evidence + marker + "\n" + modified_evidence_content
    ...
    for data_name in all_data:
        ...
        for j in range(args.n_eval):
            ...
            for iter in range(NUM_ITER):
                ...
                raw_prompts = [modify_prompt_with_evidence(data[idx]["input"]) for idx in indices]
                if data_name == 'backdoor':
                    # candidate_set 是你在 main 开头解析出的 trigger 列表 (如 ['please', 'huh'])
                    prompts = [inject_trigger_and_append_original(p, candidate_set) for p in raw_prompts]
                else:
                    # 其他情况保持原样
                    prompts = raw_prompts
